...
// 1.13tf4
//_deleteTypes2 = ["CA_Magazine", "PipeBombBase","MineBase","thingX", "ACE_Explosives_Place","StaticMGWeapon", "StaticWeapon", "ACE_MedicalLitterBase"];
_deleteTypes2 = ["CA_Magazine", "PipeBombBase","MineBase","thingX", "StaticMGWeapon", "StaticWeapon"];
_undeleteTypes = ["rhs_Flag_Russia_F","FlagCarrierUSA","Land_Cinderblocks_F","MapBoard_altis_F","Fridge_01_closed_F","Land_HBarrierBig_F"];
_zeroes = ["Door_1_rot", "Door_2_rot", "Door_3_rot", "Door_4_rot", "Door_5_rot", "Door_6_rot", "Door_7_rot", "dvere","dvere1l","dvere1r","dvere2l","dvere2r","dvere_spodni_r","dvere_spodni_l","dvere_vrchni","vrata1","vrata2","vratal1","vratar1","vratal2","vratar2","vratal3","vratar3"];  
...
_vehCOLOR = "guerilla_07";
_vehMOD = ["HideBackpacks",0,"HideBumper2",0,"HideConstruction",0,"Proxy",0,"Destruct",0];
_jeepCrewCount = 4;
...
_vehMOD = [];
_vehCOLOR = nil;
	if (attackerGearQuality == 0) then
	{
	_jeepType = "LOP_CHR_Civ_UAZ";
	_jeepCrewCount = 7;
	_vehMOD = ["light_hide",1];
	_vehCOLOR = "Camo";
	};
	if (attackerGearQuality == 1) then
	{
		_jeepType = "rhs_tigr_m_vdv";
		_jeepCrewCount = 9;
	_vehCOLOR = "Camo";
	};
	if (attackerGearQuality > 1) then
	{
		_jeepType = "rhs_tigr_sts_vdv";
		_jeepCrewCount = 8;
	_vehCOLOR = "Camo";
	};
};
...
_vehCOLOR = nil;
	if (attackerGearQuality == 0) then
	{
	_jeepType = "rhsusf_m998_w_2dr";
	_jeepCrewCount = 6;
	_vehMOD = ["hide_backTop",0,"hide_frontTop",0,"hide_snorkel",0,"hide_middleTop",1];
	_vehCOLOR = "Woodland";
	};
	if (attackerGearQuality == 1) then
	{
		_jeepType = "rhsusf_rg33_d";
		_jeepCrewCount = 8;
	};
	if (attackerGearQuality > 1) then
	{
		_jeepType = "rhsusf_rg33_m2_d";
		_jeepCrewCount = 9;
	};
};
...
if (!isDedicated) then
{
	waitUntil {sleep 0.2;!isNil "objPosHandlerClient"};
	waitUntil {sleep 0.2;!isNil "defaultInsertionPosHandler"};
	waitUntil {sleep 0.2;!isNil "vehArrHandler"};
	waitUntil {sleep 0.2;!isNil "currentVehHandler"};
};
...
while {true} do
{
	...
	{
		waitUntil {sleep 0.2;forceRoundStart || (([] call fnc_allGroupsReady) && ([] call fnc_hasPlayers))};
	};
	
	// Don't start until admin un-paused the round start.
	waitUntil {sleep 0.2;!adminPaused || forceRoundStart};
...
// 1.13tf8 закрываем двери
{_y = _x;{_y animate [format ["%1", _x], 0]} foreach _zeroes;{_y animate [format ["%1", _x], 1]} foreach _ones;} foreach ((markerPos "mrkObj1") nearobjects (deleteRadius));
// 1.13tf4	
	_toDelete = nearestObjects [markerPos "mrkObj1", _deleteTypes, deleteRadius];
	_toDelete = _toDelete + nearestObjects [markerPos "mrkObj1", _deleteTypes2, deleteRadius];
	_toDelete = _toDelete + ((markerPos "mrkObj1") nearObjects ["Default", deleteRadius]); // fix for bug with detecting satchels
	_toDelete = _toDelete + ((markerPos "mrkObj1") nearObjects ["PipeBombBase", deleteRadius]); // fix for bug with detecting m112x4 (1.13tf4)
	...
	{
	  _z = _toDelete select _i;
		if !((typeof _z) in _undeleteTypes) then
 			{deleteVehicle (_toDelete select _i);};
	};
// 1.13tf4 не удаляются мины m112x4, поэтому применяем хреновое решение:
/*	
	{      
	 _z = typeof _x;       
	 if (_z == "rhsusf_m112x4_ammo") then    
	 {    
	  deleteVehicle _x;   
	 };  
	} forEach nearestObjects [markerPos "mrkObj1", [], deleteRadius];
*/
	...
	while {_bCont} do
	{
		...
		if (_bSpawn) then
		{
			...
			for "_i" from 0 to (_vehCount - 1) do
			{
				...
				if (_vehType == _jeepType) then{
					if (count _vehMOD > 0) then {
						[_veh,_vehCOLOR,_vehMOD] call BIS_fnc_initVehicle;};
				};
				...
				if (_vehType == "B_Heli_Light_01_F") then{
