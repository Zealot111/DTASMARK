...
// 1.13tf5 
if (LastPlayersCountdown > TimeLimit) then {LastPlayersCountdown = TimeLimit;};
//
...
// 1.13tf5 
if (DefendAbility == 0) then
	{[] execVM "capture_nodefend.sqf";}
else	
	{[] execVM "capture_withdefend.sqf";};
//	
...
if (isServer) then
{
	...
	[] spawn
	{
		while {true} do 
		{ 
			waitUntil {sleep 0.5;sideEnemy countSide allUnits > 0};
			{
...
//	fnc_assigngear = compile preprocessFileLineNumbers "assigngear.sqf";
	fnc_assigngear = compile loadfile "assigngear.sqf";
	
	isJoining = false;
	
	execVM "briefing.sqf";
if (ShowPlayersMarkers==1) then
{
	execVM "unitmarkers.sqf";
}
else
{
	execVM "spawnmarkers.sqf";
};
...
//	[] spawn
//	{ 
//		[] call compile preprocessFileLineNumbers "adminactions\checkvarkick.sqf";
//	};
//};
