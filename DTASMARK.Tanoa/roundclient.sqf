Private ["_bKill","_defaultInsertionMarker", "_aRadiusMarker", "_xOffset", "_yOffset", "_pos", "_taskMessageTitle", "_taskMessageText", "_endTime"];
...
fnc_respawn =
{
	...
	//_bKill = _this select 1;
	//  if (_bKill) then {player setDamage 1;};
	...
	[_bGiveWeapons, WithGPS] call fnc_assignGear;
	player setFatigue 0;
};
...
waitUntil {sleep 0.2;!(isNil "fnc_isLeaderWithGroup")};
...
waitUntil {sleep 0.2;!(isNil "objPos")};
[] call objPosHandlerClient;

waitUntil {sleep 0.2;!(isNil "defaultInsertionPos")};
[] call defaultInsertionPosHandler;

waitUntil {sleep 0.2;alive player && (!isNil "nextAttackerSide")};
[] call objPosHandlerClient;
...
// 1.13tf2
	hint format ["Killed by " + (name _killer)];
...
[] spawn
{
	...
	while {true} do
	{
		...
		if (!enableRestrictionChecking) then
		{
			...
			waitUntil {sleep 0.2;enableRestrictionChecking};
		};
...
{
	waitUntil {sleep 0.2;!isNil "roundInProgress" || !alive player};
	if (!alive player) then
	{
		[false,false] call fnc_respawn;
	};
...
//	ace_sys_spectator_exit_spectator = nil;
	...
	{
		waitUntil {sleep 0.2;!roundInProgress || !alive player};
		if (!alive player) then
		{
			[false,false] call fnc_respawn;
		};
...
while {true} do
{
	...
	{
		waitUntil {sleep 0.2;(roundInProgress && (player getVariable ["isPlaying", false])) || !alive player};
		if (!alive player) then
		{
			[false,false] call fnc_respawn;
			player setVariable ["ready", true, true];
	...
	waitUntil {sleep 0.2;!restrictionCheckingEnabled};
	...
	if (sidePlayer == attackerSide) then
	{
		...
		{
			waitUntil {sleep 0.2;!isNil "currentVeh"};
...
//	player setDamage 0;
	...
	[] spawn
	{
		private ["_endTime"];
		_endTime = time + 30;
		waitUntil {sleep 0.2;(!(alive player)) || (!roundInProgress) || (time > _endTime)};
	...
	waitUntil {sleep 0.2;!(alive player) || !roundInProgress};
	isPlaying = false;
	player setVariable ["isPlaying", false];
	bKeepPlayerInBox = true;
	[false,false] call fnc_respawn;
